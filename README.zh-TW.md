...
| 欄位 | 意義 |
|---|---|
| `ctx 43% (168k)` | 脈絡視窗使用率，以及對話中的輸入 token 數——超過 80% 轉黃，自動壓縮會從這裡開始改寫前綴 |
...
| `keep-warm on` | [保溫策略](#keep-warm)已安裝且為最新——若升級後 CLAUDE.md 中的區塊被落下則顯示 `on*`，未安裝時 `off` 為暗色，區塊損壞時顯示 `?` |
...
warmline awake               # 免睡眠模式：抑制系統睡眠地執行一次 claude 工作
                             # 階段；/exit 後恢復正常睡眠
warmline wait-for --pid N    # 已分離的工作結束時喚醒工作階段
```
...
它所預防的重建則是約 2 倍。策略不點名任何特定排程器，只陳述需求——必須有某種東西
能在一個 TTL 之內重新進入工作階段，而且能夠撤銷——因為各個組建提供的機制並不相同。

當等待發生在*本機*時，定時 ping 並不是合適的工具。`warmline wait-for` 是一個以
背景任務執行的輪詢器：工作結束**或失敗**時它會喚醒工作階段，不需要任何排程，而且
會自行結束。定時 ping 留給本機無法觀測的等待。
...
- **一次 4.5 小時的監督等待中 96% 為 HOT，冷態重新快取的 token 為 0。** 策略開啟，
  187 個輪次，從快取讀取 1610 萬 token。所有溫度中斷都來自*自動*壓縮，沒有一次是
  TTL 過期。
- **自動壓縮在視窗約 84% 處觸發**（20 萬中的 16.7–16.9 萬，共三次）。這是保溫唯一
  無法預防、只能預警的快取殺手，因此 `ctx` 在 80% 就轉黃。
