...
| 字段 | 含义 |
|---|---|
| `ctx 43% (168k)` | 上下文窗口占用率，以及对话中的输入 token 数——超过 80% 变黄，自动压缩会从这里开始改写前缀 |
| `cache HOT (cold ~13:04)` | 上一个请求从提示缓存读取；缓存将在所示时刻过期（绿色） |
...
| `keep-warm on` | [保温策略](#keep-warm)已安装且为最新——若升级后 CLAUDE.md 中的块被落下则显示 `on*`，未安装时 `off` 为暗色，块损坏时显示 `?` |

这一行不会过时：安装器会写入 `refreshInterval: 60`，因此即使会话原地闲置，Claude
Code 也会每分钟重新运行一次这条仪表——倒计时持续走动，缓存过期后不到一分钟
`COLD(ttl?)` 就会接管，而不是让一个绿色的 `HOT` 整晚冻结在屏幕上。（这种刷新只是
本地重绘，绝不触碰 API，也不会为缓存保温。）仍有两点诚实的说明：用量数字描述的是
*上一个*请求，所以 `HOT`/`COLD(rebuilt)` 会滞后一轮；而 `COLD(ttl?)` 是基于时间的
推断——所以才有那个 `?`。
...
也会出现在其中；它们虽然无法渲染状态栏，写出的却是同样的记录文件。
...
warmline awake               # 免睡眠模式：抑制系统休眠地运行一次 claude 会话；
                             # /exit 后恢复正常休眠
warmline wait-for --pid N    # 已分离的任务结束时唤醒会话
```
...
它所预防的重建约为 2 倍。策略不点名任何具体的调度器，只陈述要求——必须有某种东西
能在一个 TTL 之内重新进入会话，并且能够被撤销——因为各个构建提供的机制并不相同。

当等待发生在*本机*时，定时 ping 并不是合适的工具。`warmline wait-for` 是一个作为
后台任务运行的轮询器：任务结束**或失败**时它会唤醒会话，不需要任何调度，并且会自行
结束。定时 ping 留给本地无法观测的等待。
...
- **一次 4.5 小时的监督等待中 96% 为 HOT，冷态重新缓存的 token 为 0。** 策略开启，
  187 个轮次，从缓存读取 1610 万 token。所有热度中断都来自*自动*压缩，没有一次是
  TTL 过期。
- **自动压缩在窗口约 84% 处触发**（20 万中的 16.7–16.9 万，共三次）。这是保温唯一
  无法预防、只能预警的缓存杀手，因此 `ctx` 在 80% 就变黄。
